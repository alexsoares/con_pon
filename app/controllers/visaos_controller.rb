class VisaosController < ApplicationController
  ...
  def fundamental
      ...
      if !(params[:search].blank?)
              @search = Professor.search(params[:search])
              @professor_rel_geral = @search.all(:joins => :unidade, :conditions => ["professors.funcao in ('PEB1 - Ensino Fundamental','PEB2 - Inglês','PEB2 - Geografia','PEB2 - Ciências','PEB2 - Português','PEB2 - História','PEB2 - Matemática','PEB2 - Ed. Física','Diretor Ed. Básica','Pedagogo')"], :order => 'pontuacao_final DESC')
      end
  end
